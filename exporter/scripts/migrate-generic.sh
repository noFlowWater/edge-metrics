#!/bin/bash
# 비Jetson systemd → DaemonSet 마이그레이션 스크립트
# 사용법: ./scripts/migrate-generic.sh
# 각 비Jetson 노드의 기존 config를 /var/lib/edge-metrics/로 복사 후 systemd 중지
...
declare -A NODE_CONFIG=(
    [rasp5]="/home/rasp5-1/ETRI/edge-metrics-exporter/config.yaml"
    [orangepi]="/home/orangepi/ETRI/edge-metrics-exporter/config.yaml"
    [lattepanda]="/home/lattepanda/ETRI/edge-metrics-exporter/config.yaml"
)

TARGET_DIR="/var/lib/edge-metrics"

echo -e "${GREEN}=== 비Jetson systemd -> DaemonSet 마이그레이션 ===${NC}"
echo ""

for NODE in rasp5 orangepi lattepanda; do
...
echo "  ./scripts/deploy.sh <VERSION> --generic"
