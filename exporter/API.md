...
Please restart the exporter to apply port changes
```

**Restart (DaemonSet):**
```bash
kubectl rollout restart ds -n monitoring edge-metrics-exporter-jetson
kubectl rollout restart ds -n monitoring edge-metrics-exporter-generic
```
...
```bash
kubectl logs -n monitoring <pod-name> -f | grep Shelly
```
...
- Check Pod logs: `kubectl logs -n monitoring <pod-name> --tail=50`
- Restart Pod if needed: `kubectl delete pod -n monitoring <pod-name>`
