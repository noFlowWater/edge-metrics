    ...
    def collect(self):
        ...
        try:
            ...
            with config_lock:
                ...
                if new_metrics:
                    # Add new metrics with enabled=True (expose by default)
...
class CombinedHandler(BaseHTTPRequestHandler):
            ...
            with config_lock:
                ...
                if current_config is None:
                    response = {
                        "status": "waiting",
                        "device_id": device_id,
                        "message": f"No config. Register via PUT /config/{device_id}"
                    }
                    self.send_response(200)
                    self.send_header('Content-Type', 'application/json')
                    self.end_headers()
    ...
    def _handle_config(self):
        ...
        try:
            ...
            with config_lock:
                config_copy = current_config.copy() if current_config else {}
...
def apply_new_config(new_config: dict):
    ...
    if old_port != new_port:
        logger.warning(
            ...
            f"Please restart the exporter to apply port changes"
        )
...
def _stub_config():
    """Return a minimal config for stub mode (no server, no local file needed)."""
    return {
        "device_type": "stub",
        "port": int(os.getenv("METRICS_PORT", "9102")),
        "reload_port": int(os.getenv("RELOAD_PORT", "9101")),
        "shelly": {"enabled": False},
        "metrics": {name: True for name in _stub_metric_names()},
    }


def _stub_metric_names():
    from collectors.stub import StubCollector
    return StubCollector.metric_names()
...
def main():
    ...
    stub_mode = os.getenv("STUB_MODE", "").lower() in ("true", "1", "yes")

    if stub_mode:
        logger.info("🧪 STUB MODE enabled — generating synthetic metrics")
        current_config = _stub_config()
        config_loader_instance = None
    else:
        config_loader_instance = ConfigLoader()
        current_config = config_loader_instance.load()

        # Waiting mode: no crash, poll until config available
        if current_config is None:
            reload_port = int(os.getenv("RELOAD_PORT", "9101"))
            start_reload_server(reload_port)  # Keep /health endpoint alive
            logger.info("Entering waiting mode - polling server every 60s...")

            while current_config is None:
                time.sleep(60)
                current_config = config_loader_instance.load()

            logger.info("Config received, initializing collectors...")
